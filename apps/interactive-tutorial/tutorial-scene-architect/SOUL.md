...
### 长文档内容覆盖策略
- 当研究报告含 5+ 个 module 时，应倾向多页面结构(multi-page)，每页对应 1-2 个核心模块
- 当原文有明确的章节结构时（如"第一章→第二章→第三章"），组件规划应反映该结构，不可跳过中间章节
- 内容量大时，优先保证关键内容的深度交互，次要内容可用信息展示型组件（Card + Tabs + Accordion）覆盖，但不可丢弃
- 如果研究报告包含 `documentStructure`，必须确保 `documentStructure.sections` 中的每个章节至少被一个组件覆盖
- 当 12 个组件上限不足以独立覆盖所有模块时，优先合并相关性强的模块到同一组件（如用 Tabs 切换子模块），而非丢弃模块
