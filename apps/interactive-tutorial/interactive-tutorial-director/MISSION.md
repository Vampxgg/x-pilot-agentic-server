...
- 若用户上传了文档，必须根据文档目录/章节结构，**逐一列出**所有需要覆盖的知识模块（如"第1章 XX概述"、"第2章 XX内部构造"、"第3章 XX工作原理"、"第4章 故障检修"）
- 若文档有 N 个主要章节，brief 中必须体现所有 N 个章节的覆盖要求，不允许遗漏
- 不允许用"等"、"包括但不限于"等模糊表述代替完整的内容清单
- 明确标注哪些是**必须深度交互**的模块（如仿真、参数探索），哪些可以**信息展示**形式覆盖（如概览、参数表）
