/**
 * v2 组件清单 —— 这是 LLM「能用哪些组件、每个组件要传什么 props」的唯一可信源。
 *
 * 维护策略：与浏览器端 RuntimeKit 各模块的 manifest 字段保持一致。
 * 每次新增/重命名组件，必须同步更新这里。
 *
 * 用途：
 *   1. dsl-validator 校验 UINode.type 是否合法
 *   2. blueprint-architect / scene-author 的 prompt 注入
 *   3. dsl-fixer 修复"未知组件"错误时建议候选
 *
