...
**用途**：通过 [Tavily](https://docs.tavily.com) 进行网页/图片搜索，返回结构化结果，支持 AI 摘要、网页正文抽取、域名过滤、时间过滤等。

> 📖 **详细文档**：[`docs/tools/web-search.md`](./web-search.md)（含全部参数、错误码、性能指标、迁移指南）

| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|------|------|------|
| `query` | string | ✅ | - | 搜索关键词（≥1 字符） |
| `num` | int (1-20) | ❌ | 5 | 请求结果数量 |
| `search_type` | `"text"` \| `"image"` | ❌ | `"text"` | 搜索模式 |
| `extract_content` | boolean | ❌ | false | 为 true 时返回每条结果的 markdown 正文 |
| `search_depth` | `"basic"` \| `"advanced"` | ❌ | `"basic"` | `"advanced"` 更深更准但更慢 |
| `topic` | `"general"` \| `"news"` \| `"finance"` | ❌ | `"general"` | 搜索主题 |
| `time_period` | `last_day` / `last_week` / `last_month` / `last_year` 等 | ❌ | - | 时间范围（细粒度自动降级到 day） |
| `gl` | string | ❌ | - | 国家代码（如 `us`、`cn`） |
| `include_domains` | string[] | ❌ | - | 仅返回这些域名 |
| `exclude_domains` | string[] | ❌ | - | 排除这些域名 |
| `timeout` | number | ❌ | 30,000 | 单次请求超时毫秒 |

> ⚠️ 旧字段 `image_size` / `image_color` / `image_type` / `aspect_ratio` / `hl` 仍接受但**已被忽略**（Tavily 不支持），保留只为向后兼容。

**输出**（成功）：
```json
{
  "query": "实际执行的 query",
  "ai_overview": "Tavily 生成的 AI 摘要回答（无则为 null）",
  "organic_results": [
    {
      "title": "网页标题",
      "link": "https://...",
      ...
      "snippet": "Tavily 给出的相关性片段",
      "date": "2026-03-12",
      "thumbnail": "",
      "images": [],
      "content": "（extract_content=true 时为完整 markdown）",
      "extracted_content": "（与 content 相同，向后兼容）",
      "score": 0.94
    }
  ],
  "response_time": 1.34
...
> 图片搜索（`search_type: "image"`）的 `organic_results` 中，`link` / `thumbnail` / `images[0]` 是同一图片 URL，`title` / `snippet` 为图片描述。

**输出**（失败）：
```json
{ "error": "Web search API key is missing. Set TAVILY_API_KEY or SEARCH_API_KEY in the environment." }
```
或
```json
...
**环境变量**：优先使用 `TAVILY_API_KEY`（Tavily，格式 `tvly-xxxxxxxx`）；未配置 Tavily 时使用 `SEARCH_API_KEY` 作为 SearchApi.io fallback。
...
```json
{
  ...
  "arguments": {
    ...
    "include_domains": ["blog.langchain.dev", "python.langchain.com"]
  }
}
```

**智能体建议**：
- 只需要事实/简答 → `extract_content: false`（更快，~2s）。
- 需要深度调研、写文章/代码 → `extract_content: true` + `search_depth: "advanced"`。
- 优先权威源 → `include_domains: ["官方文档域名"]`。
- 输出始终是合法 JSON：超过 50KB 时按结果裁剪 `content` 字段而不是粗暴截断字符串。
...
| 工具 | 环境变量 |
|------|----------|
| web_search | `TAVILY_API_KEY`；fallback 为 `SEARCH_API_KEY`（SearchApi.io） |
