...
**输出**（失败）：
```json
{ "error": "Tavily API key is missing or invalid. Set TAVILY_API_KEY in the environment." }
```
或
```json
...
**环境变量**：`TAVILY_API_KEY`（Tavily，格式 `tvly-xxxxxxxx`）。`SEARCH_API_KEY` 兼容保留：仅当其值也以 `tvly-` 开头才会被使用。
...
| 工具 | 环境变量 |
|------|----------|
| web_search | `TAVILY_API_KEY`（兼容 `SEARCH_API_KEY` 若值为 `tvly-*`） |
