...
export function createModel(options: CreateModelOptions): BaseChatModel {
  ...
  if (provider === "vertex") {
    if (!process.env.GOOGLE_APPLICATION_CREDENTIALS && !process.env.GOOGLE_API_KEY) {
      logger.warn("Vertex AI provider selected but neither GOOGLE_APPLICATION_CREDENTIALS nor GOOGLE_API_KEY is set");
    }
  ...
  switch (provider) {
    ...
    case "vertex": {
      const vertexCfg = providerConfig as { project?: string; location?: string } | undefined;
      ...
      return wrapVertexModelForSafeTools(vertexModel as unknown as BaseChatModel);
    }
